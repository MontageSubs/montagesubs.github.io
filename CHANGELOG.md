...
回应 #24 的反馈：把首页兜底页的提示文字单语化，避免以后加新语言时一路膨胀。

### 改进

- **`public/index.html` 第三层 fallback 提示单语化**：原本是 `If this doesn't work, please choose a language below.` + `如未自动跳转，请选择语言。` 中英双语并排两行；现在只保留英文一行。语种选择本来就由按钮自己的 label 承担（`简体中文` / `English` / 将来的 `繁體中文` 等，各自用对应原生 script），提示文字不需要跟着多语化——以后加 zh-hant / ja / fr 只是多一个按钮，提示文字不动。产物 2.4KB → 2.3KB。
